...
import { cartStore, getIkasOrderTotalItemCount } from "@ikas/bp-storefront";

const CartBadge = observer(function CartBadge() {
  // cartStore.cart (IkasCart, null until loaded) — there is no cartStore.order.
  // Sum quantities; .length would count distinct lines, not items.
  const count = cartStore.cart ? getIkasOrderTotalItemCount(cartStore.cart) : 0;
